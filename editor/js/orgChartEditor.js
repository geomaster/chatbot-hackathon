function init() {
    ...
    var nodeIdCounter = -1; // use a sequence to guarantee key uniqueness as we add/remove/modify nodes
        ...
        function nodeClick(e, obj) {
            var clicked = obj.part;
            if (clicked) {
                window._selected = clicked.data;
                console.log('> sel', window._selected);
                window._monaco.getModel().setValue(clicked.data.json);
            }
        }
...
function toggelClass/*tm*/(elemId, a, b) {
    var e = document.getElementById(elemId);
    e.className = toggel(a, b, e.className);
}
